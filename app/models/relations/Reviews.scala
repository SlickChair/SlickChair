...
object Reviews extends Table[Review]("REVIEWS") {
  ...
  def ins(review: Review) = DB.withSession{implicit session:Session =>
    Reviews.insert(review) }
  
  def updt(review: Review) = DB.withSession{implicit session:Session =>
    Reviews.filter(r => r.paperid === review.paperid && r.memberid === review.memberid).update(review) }
  
  def of(paper: Paper, member: Member): Option[Review] = DB.withSession{implicit session:Session =>
    Query(Reviews).filter(r => r.paperid === paper.id && r.memberid === member.id).list.headOption }
  
  def ofPaper(paper: Paper): List[Review] = DB.withSession{implicit session:Session =>
