  ...
  object Submitting extends Controller {
    ...
    def info(id: IdType) = SlickAction(IsSubmitter) { implicit r =>
      val paper: Paper = Papers.withId(Id[Paper](id))
      Ok(views.html.submissioninfo(paper, Authors.of(paper), Topics.of(paper), Navbar(r.user, Submitter)))
    }
    
    /** Displays the form to edit the informations of a given submission. */
    def edit(id: IdType) = SlickAction(IsSubmitter) { implicit r =>
      val paper: Paper = Papers.withId(Id[Paper](id))
    ...
    def doEdit(id: IdType) = SlickAction(IsSubmitter, parse.multipartFormData) { implicit r =>
